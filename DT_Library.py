...
class DecisionTree():
        ...
        if self.mode == "gain":
           # TODO: Calculate information gain for each feature
            ...
            for feature in list(X.columns):
                info_gain = self._information_Gain(feature, X, local_Y)
           ...
           # TODO: Calculate gini split for each feature
            ...
            best_feature = ""
            for feature in list(X.columns):
                gini = self._gini_Split(feature, X, local_Y)
                if gini<min_val:
                    min_val = gini
                    best_feature = feature
    ...
    def _gini_Split(self, feature, X, Y): 
        ...
        merged = np.column_stack((desired_nparray,Y_np_local))  #we have a 2-dimenstional array.the subarrays first element is for features values and the second element is for corresponding label.
